...
router.post("/", resourceValidators, async (req, res) => {
  ...
  if (!errors.isEmpty()) {
	await logEvent({
	  actorUserId: null,
	  message: "RESOURCE_CREATE_FAILED_VALIDATION: invalid resource input (400)",
	  entityType: "resource",
	  entityId: null,
	});
  ...
  try {
    ...
    await logEvent({
      actorUserId,
      message: "RESOURCE_CREATED: name=" + resourceName + ", description=" + resourceDescription + ", id=" + resourceId,
      entityType: "resource",
      entityId: resourceId,
    });
